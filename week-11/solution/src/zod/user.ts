...
import { signupSchema, signinSchema } from './path/to/schemas';

try {
  const validSignupData = signupSchema.parse(userInput);
  // Proceed with signup
} catch (error) {
  console.error(error.errors); // Handle validation errors
}

try {
  const validSigninData = signinSchema.parse(userInput);
  // Proceed with signin
} catch (error) {
  console.error(error.errors); // Handle validation errors
}
